...
bool both_neutral(int faction1, int faction2) {
    return faction1 >= 0 && faction2 >= 0
        && Factions[faction1].diplo_status[faction2] & (DIPLO_TRUCE|DIPLO_TREATY);
}
...
bool stack_search(int x, int y, int faction, StackType type, VehWeaponMode mode) {
    ...
    for (int i = 0; i < *total_num_vehicles; i++) {
        VEH* veh = &Vehs[i];
        if (veh->x == x && veh->y == y) {
            if (type == S_EnemyOneUnit && (found || !at_war(faction, veh->faction_id))) {
                return false;
            }
            found = true;
            if (type == S_NeutralOnly && (faction == veh->faction_id
            || !both_neutral(faction, veh->faction_id))) {
                return false;
            }
            if (type == S_NonPactOnly && (faction == veh->faction_id
...
void move_upkeep(int faction, UpdateMode mode) {
    ...
    if (((mode == M_Player) != is_human(faction))
    || (mode != M_Player && !thinker_enabled(faction))) {
        return;
    }
...
//                            debug("route_add %d %d -> %d %d\n", base->x, base->y, ts.rx, ts.ry);
    ...
    if (f.base_count > 0 && enemy && mode != M_Player) {
    ...
    if (DEBUG && mode == M_Visual) {
        ...
        if (k<3) {
            for (int y = 0; y < *map_axis_y; y++) {
                for (int x = y&1; x < *map_axis_x; x+=2) {
                    if (k==0) {
                        pm_overlay[x][y] = base_tile_score(x, y, 0, TRIAD_SEA);
                    } else if (k==1) {
                        pm_overlay[x][y] = former_tile_score(x, y, faction, mapsq(x, y));
                    } else if (k==2) {
...
int colony_move(const int id) {
    ...
    if (pm_safety[veh->x][veh->y] < PM_SAFE && !is_human(faction)) {
    ...
    } else if (full_search && *current_turn > 20 && !is_human(faction)) {
        AIPlans& p = plans[faction];
...
bool can_bridge(int x, int y, int faction, MAP* sq) {
    ...
    if (coast_tiles(x, y) && sq->items & BIT_BASE_RADIUS && (x&1) && (y&2)) {
...
bool can_magtube(int x, int y, int faction, MAP* sq) {
    ...
    if (sq->owner != faction && both_neutral(faction, sq->owner)) {
        return false;
    }
...
int former_move(const int id) {
    ...
    if (!full_search || is_human(faction)) {
...
bool allow_probe(int faction1, int faction2) {
    ...
    if (faction1 != faction2) {
        ...
        && !(best_reactor(faction1) != REC_FISSION
        && has_project(faction1, FAC_NETHACK_TERMINUS))) {
            return false;
        }
...
double battle_priority(int id1, int id2, int dist, int moves, MAP* sq) {
    ...
    double v2 = (sq->owner == veh1->faction_id ? (sq->is_base_radius() ? 0.15 : 0.1) : 0.0)
        ...
        + min(12, abs(offense_value(u2))) * (u2->speed() > 1 ? 0.02 : 0.01)
    ...
    */
    double v3 = min(!offense_value(u2) ? 1.3 : 1.6, v1) + clamp(v2, -0.5, 0.5)
    ...
    */
    ...
    if (veh->is_probe() && pm_enemy_dist[veh->x][veh->y] != 1) {
        ...
        while ((sq = ts.get_next()) != NULL && ts.dist <= max_dist) {
            ...
            && Vehs[id2].triad() != TRIAD_AIR) {
                ...
                } else if (ts.dist == 1 && allow_probe(faction, Vehs[id2].faction_id)
                && f->energy_credits > clamp(*current_turn * f->base_count / 8, 100, 500)
                && stack_search(ts.rx, ts.ry, faction, S_EnemyOneUnit, WMODE_COMBAT)
                && !has_abil(Vehs[id2].unit_id, ABL_POLY_ENCRYPTION)) {
                    int num = *total_num_vehicles;
                    ...
                    if (probe(id, -1, id2, 1) || num != *total_num_vehicles) {
    ...
    if (landing_unit) {
        ...
        if (veh->x == tx && veh->y == ty) {
            return mod_veh_skip(id);
        } else if (defender_count(tx, ty) < garrison_goal(tx, ty, faction, TRIAD_LAND)) {
