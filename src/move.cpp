...
bool target_priority(int faction1, int faction2);
...
bool near_ocean_shoreline(int x, int y) {
    ...
    for (const MapTile& m : iterate_tiles(x, y, 1, 9)) {
...
HOOK_API int mod_enemy_move(int id) {
    ...
    if (ai_enabled(veh->faction_id)) {
        int w = u->weapon_type;
        int triad = Chassis[u->chassis_type].triad;
        if (!mapsq(veh->x, veh->y)) {
            return SYNC;
        } else if (w == WPN_COLONY_MODULE) {
        ...
        } else if (triad == TRIAD_AIR && w <= WPN_PSI_ATTACK) {
            return aircraft_move(id);
        } else if (triad != TRIAD_AIR && (w <= WPN_PSI_ATTACK || w == WPN_PROBE_TEAM)) {
...
void invasion_plan(int faction) {
    ...
    for (int i=0; i < *total_num_bases; i++) {
        ...
        && pm_shore[base->x][base->y] >= ShoreLine) {
            path.push_back({base->x, base->y});
            debug("invasion start %2d %2d %d %d\n", base->x, base->y, p.main_region, sq->region);

        } else if (at_war(faction, base->faction_id) && !is_ocean(sq)) {
            for (const MapTile& m : iterate_tiles(base->x, base->y, 0, TableRange[3])) {
                enemy_area.insert({m.x, m.y});
            }
        }
    }
    ...
    while (++i < 4000 && k < 60 && (sq = ts.get_next()) != NULL) {
        ...
        && enemy_bases(faction, sq->region, false)
        && allow_move(ts.rx, ts.ry, faction, TRIAD_LAND)) {
            ...
            int score = min(0, pm_safety[ts.rx][ts.ry] / 4)
                + (target_priority(faction, sq->owner) ? 400 : 0)
                + (sq->region == plans[sq->owner].main_region ? 160 : 0)
                + (enemy_area.count({ts.rx, ts.ry}) ? 140 : 0)
                + (current_goal ? 120 : 0)
...
void move_upkeep(int faction, bool visual) {
    ...
    for (int y = 0; y < *map_axis_y; y++) {
        for (int x = y&1; x < *map_axis_x; x+=2) {
            sq = mapsq(x, y);
            if (sq && sq->region == p.main_sea_region) {
    ...
    for (int i=0; i < *total_num_vehicles; i++) {
        ...
        if (veh->faction_id == faction) {
            ...
            && triad == TRIAD_LAND) {
                ...
                if (sq && is_ocean(sq) && sq->is_base() && coast_tiles(veh->x, veh->y) < 8) {
...
//                            debug("route_add %d %d -> %d %d\n", base->x, base->y, ts.rx, ts.ry);
    ...
    if (f.current_num_bases > 0) {
        ...
        for (int i=0; i < *total_num_bases; i++) {
            ...
            if (sq && base->faction_id == faction) {
                int region = sq->region;
                for (int k=0; k < *total_num_bases; k++) {
                    BASE* b = &Bases[k];
                    sq = mapsq(b->x, b->y);
                    if (sq && faction != b->faction_id && !has_pact(faction, b->faction_id)) {
...
int move_to_base(int id) {
    ...
    int i = 0;
    int type = (veh_triad(id) == TRIAD_SEA ? TS_SEA_AND_SHORE : veh_triad(id));
    TileSearch ts;
...
int colony_move(int id) {
    ...
    if (can_build_base(veh->x, veh->y, faction, triad)) {
        ...
        } else if (triad != TRIAD_LAND && (at_target(veh) || !random(12))) {
            return action_build(id, 0);
        }
    }
    ...
    if (!valid_colony_path(id)) {
        ...
        int tx = -1;
        int ty = -1;
        int limit = (!((*current_turn + id) % 5) ? 500 : 200);
        TileSearch ts;
        ...
        if (tx >= 0) {
            ...
            adjust_value(pm_former, tx, ty, 2, 1);
...
int former_move(int id) {
    ...
    int i = 0;
    int limit = (!((*current_turn + id) % 5) ? 200 : 50);
    int tscore = INT_MIN;
    int action = -1;
    int tx = -1;
    int ty = -1;
    ...
    if (faction1 != faction2) {
        ...
        if (!(diplo & DIPLO_PACT)) {
            ...
            if (Factions[faction1].tech_ranking < Factions[faction2].tech_ranking)
                value += 2;
            if (MFactions[faction1].AI_fight > 0)
                value++;
            if (MFactions[faction1].AI_power > 0)
                value++;
...
bool enemy_bases(int faction, int region, bool probe) {
    for (int i=0; i < *total_num_bases; i++) {
        BASE* base = &Bases[i];
        MAP* sq = mapsq(base->x, base->y);
        if (sq && sq->region == region && (at_war(faction, base->faction_id)
...
int defender_count(int x, int y, int faction, int skip_veh_id) {
    ...
    for (int k=0; k < *total_num_vehicles; k++) {
        VEH* veh = &Vehicles[k];
        if (veh->x == x && veh->y == y && k != skip_veh_id
...
int trans_move(int id) {
    ...
    for (const MapNode& n : mapnodes) {
        if (n.type == NODE_NEED_FERRY) {
            need_ferry = true;
        }
    }
    int limit = (need_ferry ? 1600 : 200);
    int i = 0;
    int tx = -1;
    int ty = -1;
    TileSearch ts;
    ...
    while (++i < limit && (sq = ts.get_next()) != NULL) {
        if (non_combat_move(ts.rx, ts.ry, veh->faction_id, TRIAD_SEA)) {
            ...
            } else if (i < 200 && !sq->is_visible(veh->faction_id)
...
int aircraft_move(int id) {
    ...
    int max_dist = speed - veh->road_moves_spent;
    bool at_base = sq->is_base();

    if (!at_target(veh)) {
        return SYNC;
    }
    if (veh->road_moves_spent >= speed) {
        return move_to_base(id);
    }
    if (need_heals(veh) && at_base) {
        max_dist = 1;
    }
    int i = 0;
    int tx = -1;
    int ty = -1;
    int id2 = -1;
    double best_odds = 1.1 - 0.007 * min(100, p.aircraft);
    TileSearch ts;
    ts.init(veh->x, veh->y, TRIAD_AIR);

    while (++i < 440 && (sq = ts.get_next()) != NULL && ts.dist <= max_dist) {
        ...
        if (at_war(faction, faction2) && (id2 = choose_defender(ts.rx, ts.ry, faction, id)) >= 0) {
            ...
            double v1 = battle_eval(id, id2, mov_rate, mov_rate);
            double v2 = (sq->owner == faction ? (sq->is_base_radius() ? 0.2 : 0.1) : 0.0)
                + (to_base || !conf.collateral_damage_value ? 0.0 : 0.05 * (pm_enemy[ts.rx][ts.ry]))
                + min(12, abs(offense_value(u2)))*0.02
                - 0.02*ts.dist;
...
bool attack_search(int id, int max_dist, int* tx, int* ty) {
    VEH* veh = &Vehicles[id];
    int faction = veh->faction_id;
    int triad = unit_triad(veh->unit_id);
    int type = (triad == TRIAD_SEA ? TS_SEA_AND_SHORE : triad);
    int skip = (triad == TRIAD_LAND ? 2 : 0);
    int tolerance = (triad == TRIAD_LAND ? 1 : 2) + (!pm_enemy_near[veh->x][veh->y] ? 2 : 0);
    int priority = 0;
    int path_node = 0;
    int path_dist = 0;
    int i = 0;
    ...
    *ty = -1;
    ...
    while (++i < 2000 && (sq = ts.get_next()) != NULL && ts.dist <= max_dist) {
        if (sq->is_base()
        && (at_war(faction, sq->owner) || (veh->is_probe() && allow_probe(faction, sq->owner)))
        && (!priority || target_priority(faction, sq->owner))) {
            max_dist = (*tx < 0 ? ts.dist + tolerance : max_dist);
            *tx = ts.rx;
            *ty = ts.ry;
            path_node = ts.cur;
            path_dist = ts.dist;
    ...
    if (*tx >= 0) {
        PathNode& prev = ts.paths[ts.paths[path_node].prev];
        if (!veh->is_probe()) {
            int id2;
            if (path_dist < 3 && (id2 = choose_defender(*tx, *ty, faction, id)) >= 0
            && battle_eval(id, id2, 1, 1) < 0.7) {
                ...
                for (auto m : iterate_tiles(veh->x, veh->y, 0, 9)) {
                    ...
                    && at_war(faction, m.sq->owner)) {
                        if (m.sq->items & (TERRA_SENSOR | TERRA_BUNKER | TERRA_FOREST) && !random(2)) {
                            *tx = m.x;
                            *ty = m.y;
            ...
            if (path_dist > 3 && pm_enemy_near[*tx][*ty] > 2) {
...
bool airdrop_move(int id, MAP* sq) {
    VEH* veh = &Vehicles[id];
...
int combat_move(int id) {
    ...
    */
    ...
    // TODO: more accurate scouting priority
    ...
    // Tolerate worse odds the more there are expendable units available.
    double best_odds = (at_base && defenders < 1 ? 1.4 : 1.2) - (at_enemy ? 0.2 : 0)
    ...
    int search_limit = (triad == TRIAD_SEA ? 60 : 40);
    bool defend;
    if (triad == TRIAD_SEA) {
    ...
    if (veh->is_probe() && !defend) {
        attack_search(id, search_limit, &tx, &ty);
        if (tx >= 0) {
            return set_move_to(id, tx, ty);
        }
    }
