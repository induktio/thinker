...
bool valid_colony_path(int id) {
    ...
    if (triad == TRIAD_LAND && map_range(veh->x, veh->y, tx, ty) > 14 - veh->iter_count) {
        return false;
    }
    if (at_target(veh) || ~veh->flags_1 & 0x40000 || !can_build_base(tx, ty, fac, triad)) {
        return false;
    }
    return true;
}
